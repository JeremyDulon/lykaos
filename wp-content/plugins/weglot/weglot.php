...
class Weglot {
	...
	function translatePageTo($final,$l) { 
		...
		for($i=0;$i<count($out[0]);$i++) {
			...
			if((($current_url[0] == 'h' && $parsed_url['host']==$_SERVER['HTTP_HOST']) || $current_url[0] =='/') 
				&& strpos($current_url,$admin_url) === false && strpos($current_url,'wp-login') === false && !WGUtils::endsWith($current_url,'.jpg') && !WGUtils::endsWith($current_url,'.png') && !WGUtils::endsWith($current_url,'.pdf')
	...
	public function returnWidgetCode($forceNoMenu = false) { 
		...
		$button = '<!--Weglot '.WEGLOT_VERSION.'-->'.$aside1.'<'.$tag.' class="wgcurrent wg-li '.$flag_class.$current.'"><a href="javascript:void(0);">'.($withname ? ($full? WGUtils::getLangNameFromCode($current,false):strtoupper($current)):"").'</a></'.$tag.'>'.$list.$aside2;
